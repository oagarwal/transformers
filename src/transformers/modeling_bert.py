...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        return outputs  # (loss), scores, sequence_output, (hidden_states), (attentions)



@add_start_docstrings(
    """Bert Model with a token classification head on top (a linear layer on top of
    the hidden-states output) supporting multiple labels per token.
...
class BertForMultiLabelTokenClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length, config.num_classes)`, `optional`, defaults to :obj:`None`):
            Labels for computing the token classification loss.``.
    ...
    Examples::
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                active_loss = attention_mask.view(-1) == 1
                active_loss = torch.transpose(active_loss.repeat((self.num_labels, 1)), 0, 1)
                active_labels = torch.where(
                    active_loss, labels.view(-1, self.num_labels), torch.tensor(0).type_as(labels)
                )
            else:
                active_labels = labels.view(-1, self.num_labels)
