...
MODEL_FOR_TOKEN_MULTILABEL_CLASSIFICATION_MAPPING = OrderedDict(
    [
        (BertConfig, BertForMultiLabelTokenClassification),
    ]
)
...
class AutoModelForMultiLabelTokenClassification:
    r""" 
        :class:`~transformers.AutoModelForMultiLabelTokenClassification` is a generic model class
        that will be instantiated as one of the multi-label token classification model classes of the library
        when created with the `AutoModelForMultiLabelTokenClassification.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelForMultiLabelTokenClassification is designed to be instantiated "
            "using the `AutoModelForMltiLabelTokenClassification.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelForMultiLabelTokenClassification.from_config(config)` methods."
        )    
    ...
    def from_config(cls, config):
        ...
        Args:
            config (:class:`~transformers.PretrainedConfig`):
                ...
                - isInstance of `bert` configuration class: :class:`~transformers.BertModelForMultiLabelTokenClassification` (Bert model)

        Examples::

            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelForMultiLabelTokenClassification.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_TOKEN_MULTILABEL_CLASSIFICATION_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        ...
        raise ValueError(
            ...
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_TOKEN_MULTILABEL_CLASSIFICATION_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        The model class to instantiate is selected as the first pattern matching
        in the `pretrained_model_name_or_path` string (in the following order):
            - contains `bert`: :class:`~transformers.BertForMultiLabelTokenClassification` (Bert model)
        ...
        Examples::
            ...
            model = AutoModelForiMultiLabelTokenClassification.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache.
            model = AutoModelForMultiLabelTokenClassification.from_pretrained('./test/bert_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelForMultiLabelTokenClassification.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
