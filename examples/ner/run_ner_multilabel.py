...
from transformers.modeling_auto import MODEL_FOR_TOKEN_MULTILABEL_CLASSIFICATION_MAPPING
from utils_ner_multilabel import convert_examples_to_features, get_labels, read_examples_from_file
...
MODEL_CONFIG_CLASSES = list(MODEL_FOR_TOKEN_MULTILABEL_CLASSIFICATION_MAPPING.keys())
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = AutoModelForMultiLabelTokenClassification.from_pretrained(checkpoint)
            model.to(args.device)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        tokenizer = AutoTokenizer.from_pretrained(args.output_dir, **tokenizer_args)
        model = AutoModelForMultiLabelTokenClassification.from_pretrained(args.output_dir + args.predict_ckpt)
        model.to(args.device)
        ...
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "test.txt"), "r") as fr:
                example_id = 0
                words = []
                labels = []
                for line in fr:
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        ...
                        tag = " ".join([s.replace(" ", "_") for s in line.split("\t")[1:]])
                        output_line = line.split("\t")[0] + "\t" + tag + "\t" + predictions[example_id].pop(0)
