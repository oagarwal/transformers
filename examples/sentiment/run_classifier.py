...
""" Fine-tuning the library models for binary classification (Bert or Roberta). """
...
from utils_classifier import convert_examples_to_features, get_labels, read_examples_from_file
...
def main():
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        tokenizer = AutoTokenizer.from_pretrained(args.output_dir, **tokenizer_args)
        model = AutoModelForSequenceClassification.from_pretrained(args.output_dir + args.predict_ckpt)
        model.to(args.device)
        ...
        with open(output_test_predictions_file, "w") as writer:
            with gzip.open(os.path.join(args.data_dir, "test.jsonl.gz")) as f:
                for line, prediction in zip(f, predictions):
